...
def do_train(model, data_loader, criterion, optimizer, device, config, epoch, grad_acc=1):
        ...
        for idx, (inputs) in tqdm(enumerate(data_loader), total=len(data_loader)):
            ...
            # data, targets = mixup(x1, grapheme_root, vowel_diacritic, consonant_diacritic, 0.4)
            # logit_grapheme_root2, logit_vowel_diacritic2, logit_consonant_diacritic2 = model(data)
            # loss_gr2, loss_vd2, loss_cd2 = mixup_criterion(logit_grapheme_root2, logit_vowel_diacritic2, logit_consonant_diacritic2, targets, criterion)
     
            logit_grapheme_root3, logit_vowel_diacritic3, logit_consonant_diacritic3 = model(x)
            ...
            # logit_grapheme_root = (logit_grapheme_root + logit_grapheme_root2) / 2.
            # logit_vowel_diacritic = (logit_vowel_diacritic + logit_vowel_diacritic2) / 2.
            # logit_consonant_diacritic = (logit_consonant_diacritic + logit_consonant_diacritic2) / 2.

            # loss_gr = (loss_gr + loss_gr2) / 2.
            # loss_cd = (loss_cd + loss_cd2) / 2.
            # loss_vd = (loss_vd + loss_vd2) / 2.
...
def do_eval(model, data_loader, criterion, device):
    ...
    with torch.no_grad():
        ...
        for inputs in tqdm(data_loader, total=len(data_loader)):
            ...
            total_loss += 8 * loss_gr.item() + loss_vd.item() + 2 * loss_cd.item()
...
def run(config_file):
    ...
    if config.train.resume:
        ...
        checkpoint = torch.load(config.train.path)
